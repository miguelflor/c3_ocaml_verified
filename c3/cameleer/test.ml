...
let compute_linearization parent_map universe cls =
  ...
  MRO.c3_linearization universe cls
...
let tests = [
  ...
  };
  {
    ...
    test_class = "Z";
    universe = ["A";"C";"B";"O";"D";"E";"Z"; "K1";"K2";"K3"];
...
let run_tests () =
  List.iter (fun test ->
    ...
    | ExpectSuccess expected_list ->
        begin
          try
            let result = compute_linearization test.parent_map test.universe test.test_class in
    ...
    | ExpectFail ->
        begin
          try
            let result = compute_linearization test.parent_map test.universe test.test_class in
            Printf.printf "%s: FAILED (expected Invalid_argument but got success with the list %s)\n"
             test.description
             (String.concat " " result);
            exit 1
          with
          | Invalid_argument _ ->
              Printf.printf "%s: passed (raised Invalid_argument as expected)\n" test.description
          | e ->
