...
\author{
  \IEEEauthorblockN{Miguel Flor, António Ravara and Mário Pereira}
  \IEEEauthorblockA{NOVA LINCS, Nova School of Science and Technology\\ m.flor@campus.fct.unl.pt, \{aravara, mjp.pereira\}@fct.unl.pt}
}
...
This is where the C3 linearization algorithm comes into play, as it provides a consistent and predictable way to resolve this conflict, the algorithm says that we need to prioritize $B$ over $C$, because $B$ comes first (reading from left to right) in the class hierarchy, and therefore the MRO of $A$ will be $(A, B, C, D)$.

\subsection{C3 Linearization Algorithm}
The C3 algorithm produces an MRO that is consistent with the extended precedence graph (EPG), local precedence order (LPO), and monotonicity \cite{barrettMonotonicSuperclassLinearization1996} (all of these properties are formally defined in section \ref{subsec:PropertiesformalSpec}).
...
For $m,n \geq 2$.
\begin{align*}
&\forall\,i,j\;\bigl(0\le i<j\le n \;\Longrightarrow \\
&\quad \exists\,p,q\;\bigl(0\le p<q\le m \wedge C_p=P_i \wedge C_q=P_j\bigr)\bigr)
\end{align*}
